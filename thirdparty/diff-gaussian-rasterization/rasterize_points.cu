	...
	const bool debug)
{
  ...
  torch::Tensor out_language_feature = torch::full({NUM_CHANNELS_language_feature, H, W}, 0.0, float_opts);

  torch::Tensor out_depth = torch::full({1, H, W}, 0.0, float_opts);
  torch::Tensor out_alpha = torch::full({1, H, W}, 0.0, float_opts);
  torch::Tensor radii = torch::full({P}, 0, means3D.options().dtype(torch::kInt32));
  torch::Tensor out_top3_idx = torch::full({5, H, W}, 0, int_opts);  
  torch::Tensor out_top3_contrib = torch::full({5, H, W}, 0.0, float_opts);
  ...
  if(P != 0)
  {
	  ...
	  rendered = CudaRasterizer::Rasterizer::forward(
		...
		out_top3_idx.contiguous().data<int>(),
		out_top3_contrib.contiguous().data<float>(),
		radii.contiguous().data<int>(),
		debug);
  }
  return std::make_tuple(rendered, out_color, out_depth, out_alpha, radii, geomBuffer, binningBuffer, imgBuffer, out_top3_idx, out_top3_contrib, out_language_feature);
}
